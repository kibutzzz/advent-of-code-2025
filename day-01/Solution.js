...
export default class Solution {
    ...
    solve2(text) {
        ...
        for (const instruction of instructions) {
            const sign = instruction.direction === 'R' ? 1 : -1;
            dial = calculateDialValueAfterFullRotation(
                dial,
                instruction.amount * sign,
                rotationsObject
            );
        }
...
function calculateDialValueAfterFullRotation(
    currentValue,
    change,
    rotationsObject
) {
    ...
    if (possibleNewValue >= MAX_DIAL_VALUE) {
        ...
        return calculateDialValueAfterFullRotation(
            MIN_DIAL_VALUE,
            possibleNewValue - MAX_DIAL_VALUE,
            rotationsObject
        );
    }
    ...
    if (possibleNewValue < MIN_DIAL_VALUE) {
        ...
        return calculateDialValueAfterFullRotation(
            MAX_DIAL_VALUE,
            possibleNewValue - MIN_DIAL_VALUE,
            rotationsObject
        );
    }
